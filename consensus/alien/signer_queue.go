...
func (s *Snapshot) createSignerQueue() ([]common.Address, error) {
	...
	if s.Number+1 > PosNewEffectNumber {
		return s.createSignerNewQueue()
	}
...
func (s *Snapshot) buildTallyMinerV2() TallySlice {
	var tallySlice TallySlice
	for address, stake := range s.TallyMiner {
...
func (s *Snapshot) buildTallySliceV2() TallySlice {
	var tallySlice TallySlice
	for address, stake := range s.Tally {
		if !candidateNeedPD || s.isCandidate(address) {
...
func (s *Snapshot) createSignerNewQueue() ([]common.Address, error) {
	...
	if (s.Number+1)%(s.config.MaxSignerCount*s.LCRS) == 0 {
		...
		mainMinerSlice := s.buildTallySliceV2()
		sort.Sort(TallySlice(mainMinerSlice))
		secondMinerSlice := s.buildTallyMinerV2()
		//sort.Sort(TallySlice(secondMinerSlice))
		queueLength := int(s.config.MaxSignerCount)
		mainSignerSliceLen := len(mainMinerSlice)
		if queueLength >= defaultOfficialMaxSignerCount {
			mainMinerNumber := (9*queueLength + defaultOfficialMaxSignerCount - 1) / defaultOfficialMaxSignerCount
			secondMinerNumber := 12 * queueLength / defaultOfficialMaxSignerCount
			secondMinerSlice=s.selectSecondMinerByPre(secondMinerSlice,secondMinerNumber)
			sort.Sort(TallySlice(secondMinerSlice))
			secondLength := len(secondMinerSlice)
			if secondLength== 0 {
				mainMinerNumber = queueLength - secondLength
			}else if secondMinerNumber >= secondLength {
				signerSlice = s.selectSecondMinerNewInsufficient(secondMinerSlice, signerSlice,secondMinerNumber)
			} else {
				mainMinerNumber = queueLength - secondMinerNumber
				signerSlice = s.selectSecondMiner(secondMinerSlice, secondMinerNumber, signerSlice, queueLength)
			}
			mainMinerSlice=s.selectMainMinerByPre(mainMinerSlice)
			sort.Sort(TallySlice(mainMinerSlice))
			mainSignerSliceLen = len(mainMinerSlice)
			// select Main Miner
			signerSlice = s.selectMainMiner(mainMinerNumber, mainSignerSliceLen, signerSlice, mainMinerSlice, secondMinerNumber)
...
func (s *Snapshot) selectMainMinerByPre( miners TallySlice ) TallySlice{
	if len(miners) <=posMaxMainCandidateNum{
		return miners
	}
	totalAmount :=big.NewInt(0)
	...
	sort.Sort(tallySlice)
	if len(tallySlice) <=posMaxMainCandidateNum {
		return tallySlice
	}
	return tallySlice[0:posMaxMainCandidateNum]
}

func (s *Snapshot) selectSecondMinerByPre(candidatePledgeSlice TallySlice,secondMinerNumber int) TallySlice{
...
func (s *Snapshot) selectSecondMinerNewInsufficient(tallyMiner TallySlice, signerSlice SignerSlice,secondMinerNumber int) SignerSlice {
